...
import com.github.skozlov.commons.scala.lang.{RichMatcher, RichString, checkNumberFormatRadix}

import java.util.regex.Pattern
import scala.annotation.tailrec
import scala.collection.mutable.{LinkedHashSet, StringBuilder}

case class Rational private(numerator: BigInt, denominator: BigInt) extends Real with Ordered[Rational] {
    ...
    lazy val fractionalPart: Rational = new Rational(numerator % denominator, denominator)
    ...
    def toPeriodic(radix: Int = 10): String = {
        ...
        if (this.isWhole) {
            ...
            val fractionalPartResult = formatFractionalPart(new StringBuilder(), LinkedHashSet(), initialRemainder)
...
object Rational {
    ...
    private val commonFractionPattern = Pattern.compile("""^(?<num>-?[0-9a-zA-Z]+)/(?<den>[0-9a-zA-Z]+)$""")

    private val mixedFractionPattern = Pattern.compile(
    """^(?<int>-?[0-9a-zA-Z]+) (?<num>[0-9a-zA-Z]+)/(?<den>[0-9a-zA-Z]+)$"""
    )

    private val nonPeriodicPattern = Pattern.compile("""^(?<int>-?[0-9a-zA-Z]+)\.(?<frac>[0-9a-zA-Z]+)$""")
    ...
    def apply(serialized: String, radix: Int = 10): Rational = {
        ...
        intPattern.matcher(serialized).ifMatches {_ =>
        ...
        }).applyOrElse[Unit, Rational]((), _ =>
            throw new NumberFormatException(s"Cannot parse rational number: $serialized")
        )
    }
}
