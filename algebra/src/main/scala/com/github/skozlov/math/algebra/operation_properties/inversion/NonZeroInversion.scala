...
trait NonZeroInversion[A, Op <: (A, A) => A, ZeroOp <: (A, A) => A]
    extends PartialInversion[A, Op]

object NonZeroInversion {
  def apply[A, Op <: (A, A) => A, ZeroOp <: (A, A) => A](
      invert: A => A
  )(implicit zero: Id[A, ZeroOp]): NonZeroInversion[A, Op, ZeroOp] = {
