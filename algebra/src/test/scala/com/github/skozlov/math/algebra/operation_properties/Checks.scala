package com.github.skozlov.math.algebra.operation_properties

import com.github.skozlov.math.algebra.operation_properties.id.{
  Id,
  LeftId,
  RightId,
}
...
trait Checks extends Matchers {
  ...
  def checkCommutativity[A, B, Op <: (A, A) => B](
      op: Op,
      checkEquality: (B, B) => Any = { (a: B, b: B) => a shouldBe b },
  )(operands: A*): Unit = {
    val ops = operands.distinct
    for {
      j <- ops.indices
  ...
  def checkAssociativity[A, Op <: (A, A) => A](
    ...
    checkAssociativityOnTriples(op, checkEquality)(
  ...
  def checkPartialInversion[A, Op <: (A, A) => A](
      ...
      nonInvertible: A*
  )(implicit id: Id[A, Op], inversion: PartialInversion[A, Op]): Unit = {
  ...
  def checkNonZeroInversion[A, Op <: (A, A) => A, ZeroOp <: (A, A) => A](
      ...
      id: Id[A, Op],
      zero: Id[A, ZeroOp],
      inversion: NonZeroInversion[A, Op, ZeroOp],
  ): Unit = {
    val _zero = zero.id
  ...
  def checkFullInversion[A, Op <: (A, A) => A](
      ...
      operands: A*
  )(implicit id: Id[A, Op], inversion: FullInversion[A, Op]): Unit = {
    checkPartialInversion(op, checkEquality)(operands: _*)()
  }

  def checkLeftDistributivityOnTriples[
      A,
      B,
      Add <: (B, B) => B,
      Mul <: (A, B) => B,
  ...
  def checkRightDistributivityOnTriples[
      A,
      B,
      Add <: (A, A) => A,
      Mul <: (A, B) => A,
  ...
  def checkDistributivity[A, Add <: (A, A) => A, Mul <: (A, A) => A](
    ...
    checkDistributivityOnTriples(add, mul, checkEquality)(
      operandsToTriples(operands): _*
    )
  }
}
