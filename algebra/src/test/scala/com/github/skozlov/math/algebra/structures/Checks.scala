...
trait Checks
    extends com.github.skozlov.math.algebra.operation_properties.Checks {
  def checkMonoidOnTriples[A, Op <: (A, A) => A](
      op: Op,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: (A, A, A)*)(implicit monoid: Monoid[A, Op]): Unit = {
  ...
  def checkMonoid[A, Op <: (A, A) => A](
      op: Op,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: A*)(implicit monoid: Monoid[A, Op]): Unit = {
  ...
  def checkGroupOnTriples[A, Op <: (A, A) => A](
      op: Op,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: (A, A, A)*)(implicit group: Group[A, Op]): Unit = {
  ...
  def checkGroup[A, Op <: (A, A) => A](
      op: Op,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: A*)(implicit group: Group[A, Op]): Unit = {
  ...
  def checkRingOnTriples[A, Add <: (A, A) => A, Mul <: (A, A) => A](
      ...
      mul: Mul,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: (A, A, A)*)(implicit ring: Ring[A, Add, Mul]): Unit = {
  ...
  def checkFieldOnTriples[A, Add <: (A, A) => A, Mul <: (A, A) => A](
      ...
      mul: Mul,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: (A, A, A)*)(implicit field: Field[A, Add, Mul]): Unit = {
    implicit val additionId: Id[A, Add] = field.additionGroup.id
    implicit val multiplicationId: Id[A, Mul] = field.multiplicationId
    implicit val multiplicationInversion: NonZeroInversion[A, Mul, Add] =
      field.multiplicationInversion
    ...
    checkNonZeroInversion[A, Mul, Add](mul, checkEquality)(
      flattenTriples(operands): _*
    )
  }
  ...
  def checkField[A, Add <: (A, A) => A, Mul <: (A, A) => A](
      ...
      mul: Mul,
      checkEquality: (A, A) => Any = { (a: A, b: A) => a shouldBe b },
  )(operands: A*)(implicit field: Field[A, Add, Mul]): Unit = {
    checkFieldOnTriples(add, mul, checkEquality)(
      operandsToTriples(operands): _*
    )
  }
}
